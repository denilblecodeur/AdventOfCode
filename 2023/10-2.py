...
grid[i] = grid[i][:j] + '|' + grid[i][j+1:] # remplacer 'S' par '|' !!

vis = [[0] * len(grid[0]) for _ in range(len(grid))]
for i in range(len(grid)):
    for j in range(len(grid[0])):
        if dist[i][j] == 0:
            vis[i][j] = sum(dist[i][k] for k in range(j) if grid[i][k] in "|LJ") % 2

print(sum(sum(vis[i][j] for j in range(len(grid[0]))) for i in range(len(grid))))